...
const API_ROOT = "/api/v1/"; // root of the api url
...
const createAPI = (apiRoot) => {
  return {
    get: (url, headers = {}) =>
      to(
        axios
          .get(`${apiRoot}${url}`, axiosConfig(headers))
          .then(responseBody)
          .catch(errBody)
      ),
    post: (url, body, headers = {}) =>
      to(
        axios
          .post(`${apiRoot}${url}`, body, axiosConfig(headers))
          .then(responseBody)
          .catch(errBody)
      ),
    patch: (url, body, headers = {}) =>
      to(
        axios
          .patch(`${apiRoot}${url}`, body, axiosConfig(headers))
          .then(responseBody)
          .catch(errBody)
      ),
  };
